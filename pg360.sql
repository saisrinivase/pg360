...
\qecho '  --font-ui: Helvetica, Arial, sans-serif;'
\qecho '  --space-1: 4px;'
\qecho '  --space-2: 8px;'
\qecho '  --space-3: 10px;'
\qecho '  --radius-sm: 0;'
\qecho '  --font-body: 16px;'
\qecho '  --font-title: 22px;'
\qecho '  --font-subtitle: 18px;'
\qecho '}'
\qecho ''
\qecho '* {'
...
\qecho '  --space-2: 7px;'
\qecho '  --space-3: 9px;'
\qecho '  --font-body: 14px;'
\qecho '  --font-title: 20px;'
\qecho '  --font-subtitle: 16px;'
\qecho '}'
\qecho ''
...
\qecho ''
\qecho 'pre {'
\qecho '  font: 14px/1.45 var(--font-ui);'
...
\qecho '  font-size: var(--font-title);'
...
\qecho '  font-size: calc(var(--font-title) - 2px);'
\qecho '  font-weight: 700;'
\qecho '  color: var(--accent-blue);'
\qecho '}'
\qecho ''
...
\qecho '  font-size: 13px;'
\qecho '  font-family: Helvetica, Arial, sans-serif;'
...
\qecho '  border-bottom: 1px solid var(--accent-blue);'
\qecho '  color: #000000;'
\qecho '  font: 13px/1.45 var(--font-ui);'
\qecho '  white-space: pre-wrap;'
\qecho '}'
\qecho ''
\qecho '.report-runline {'
\qecho '  margin: 4px 0 10px;'
\qecho '  padding: 2px 0 10px;'
\qecho '  border-bottom: 1px solid var(--accent-blue);'
\qecho '  color: #000000;'
...
\qecho '  display: none;'
\qecho '  font: 600 12px Helvetica, Arial, sans-serif;'
\qecho '  color: #000000;'
\qecho '  background: transparent;'
\qecho '  border: 0;'
\qecho '  border-radius: 0;'
...
\qecho '  border: 0;'
\qecho '  color: var(--accent-blue);'
\qecho '  font: 700 11px Helvetica, Arial, sans-serif;'
...
\qecho '  -webkit-overflow-scrolling: touch;'
\qecho '  overscroll-behavior-x: contain;'
\qecho '  border: 1px solid var(--line);'
\qecho '  background: #ffffff;'
...
\qecho '}'
\qecho ''
\qecho '.table-row-toggle:disabled {'
\qecho '  cursor: default;'
\qecho '  color: var(--muted);'
\qecho '  background: var(--panel);'
\qecho '}'
\qecho ''
\qecho 'tbody tr.row-collapsed {'
\qecho '  display: none;'
\qecho '}'
\qecho ''
\qecho 'table.pg360,'
...
\qecho '  padding: 28px var(--space-2) var(--space-1);'
\qecho '  font-family: var(--font-ui);'
\qecho '  font-size: 11px;'
\qecho '  white-space: pre-wrap;'
\qecho '  overflow-x: auto;'
...
\qecho '  }'
\qecho ''
\qecho '  .table-wrap table.pg360,'
\qecho '  .table-wrap table {'
\qecho '    min-width: 640px;'
\qecho '  }'
\qecho '}'
\qecho ''
\qecho '</style>'
...
SELECT
  '<div class="report-runline">dbname:' ||
  replace(replace(replace(replace(replace(current_database(),'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') ||
  ' version:' ||
  replace(replace(split_part(version(),' ',2),'<','&lt;'),'>','&gt;') ||
  ' host:' ||
...
WITH provider_ctx AS (
  SELECT
    ...
    COALESCE(current_setting('data_directory', true), '') AS data_directory,
    COALESCE(current_setting('rds.extensions', true), '') AS rds_extensions,
    COALESCE(current_setting('azure.extensions', true), '') AS azure_extensions,
    COALESCE(current_setting('shared_preload_libraries', true), '') AS shared_preload_libraries,
    COALESCE(current_setting('pg_stat_statements.track', true), '') AS pgss_track
),
provider_det AS (
  SELECT
    CASE
...
summary AS (
  SELECT
    COUNT(*) AS installed_cnt,
    COUNT(*) FILTER (WHERE pg_trust = 'Trusted' OR extname = 'plpgsql') AS trusted_common_cnt,
    COUNT(*) FILTER (WHERE review_posture IN ('MEDIUM','HIGH')) AS admin_review_cnt,
    COUNT(*) FILTER (WHERE review_posture = 'HIGH') AS high_review_cnt,
    (SELECT COUNT(*)
     FROM ext_catalog c
     WHERE c.ext_class = 'Diagnostic'
       AND c.recommendation_tier IN ('Required','Recommended')
       AND NOT EXISTS (SELECT 1 FROM ext e WHERE e.extname = c.extname)
    ) AS missing_diagnostic_cnt
  FROM ext_eval
)
SELECT
  '<div class="card-grid">' ||
...
-- S05.4 Sequence exhaustion / runway check
\qecho '<div class="subsection">'
\qecho '<div class="subsection-title">Sequence Exhaustion and Runway Check</div>'
SELECT '<div class="finding info"><div class="finding-body">Approximate runway based on current pg_sequences values. Remaining values can move quickly under concurrent nextval activity, and cached or cycling sequences need manual interpretation. Use this as an early warning to catch migrated INT or NUMBER keys before inserts fail.</div></div>';
\qecho '<div class="table-wrap">'
\qecho '<table class="pg360"><thead><tr>'
\qecho '<th>Sequence</th><th>Table</th><th>Column</th><th>Data Type</th><th>Last Value</th><th>Ceiling</th><th>Remaining Values</th><th>Headroom %</th><th>Status</th><th>Suggested Action</th>'
\qecho '</tr></thead><tbody>'
...
SELECT
  COALESCE(
    string_agg(
      ...
      '<td>' || replace(replace(replace(replace(replace(COALESCE(tbl,''),'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '<td>' || replace(replace(replace(replace(replace(COALESCE(col,''),'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '<td>' || replace(replace(replace(replace(replace(COALESCE(data_type,'unknown'),'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '<td class="num">' || COALESCE(to_char(last_value,'FM999,999,999,999,999,999,999,999'),'NULL') || '</td>' ||
      '<td class="num">' || COALESCE(to_char(max_value,'FM999,999,999,999,999,999,999,999'),'N/A') || '</td>' ||
      CASE
        WHEN remaining_values IS NULL THEN '<td class="num warn">N/A</td>'
        ELSE '<td class="num ' ||
             CASE WHEN status = 'CRITICAL' THEN 'crit' WHEN status = 'WARN' THEN 'warn' ELSE '' END ||
             '">' || to_char(remaining_values,'FM999,999,999,999,999,999,999,999') || '</td>'
      END ||
      CASE
        WHEN headroom_pct IS NULL THEN '<td class="num warn">N/A</td>'
        ELSE '<td class="num ' ||
             CASE WHEN status = 'CRITICAL' THEN 'crit' WHEN status = 'WARN' THEN 'warn' ELSE '' END ||
             '">' || to_char(headroom_pct,'FM990D0000') || '%</td>'
      END ||
      '<td>' ||
      CASE status
        WHEN 'CRITICAL' THEN '<span class="severity-pill pill-critical">LOW RUNWAY</span>'
        WHEN 'WARN' THEN '<span class="severity-pill pill-warning">PLAN NOW</span>'
        WHEN 'OK' THEN '<span class="severity-pill pill-good">OK</span>'
        ELSE '<span class="severity-pill pill-info">VERIFY</span>'
      END ||
      '</td>' ||
      '<td>' || replace(replace(replace(replace(replace(guidance,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '</tr>',
      ''
    ),
    '<tr><td colspan="10" class="table-empty">No non-system sequences with visible last_value found for runway analysis.</td></tr>'
  )
FROM (
  SELECT
    seq_name,
    ...
    status,
    CASE
      WHEN cycle THEN 'Cycling sequence. Verify duplicates are acceptable before relying on remaining range.'
      WHEN remaining_values IS NULL THEN 'Verify sequence bounds and ownership manually.'
      WHEN remaining_values <= 1000 OR COALESCE(headroom_pct, 100) <= 1 THEN 'Near exhaustion. Review bigint widening, sequence max, and application key assumptions now.'
      WHEN remaining_values <= 100000 OR COALESCE(headroom_pct, 100) <= 5 THEN 'Low headroom. Plan key-space extension before the next growth cycle.'
      ELSE 'Runway looks healthy in the current snapshot.'
    END AS guidance
  FROM (
    SELECT
      seq_name,
      ...
      cycle,
      CASE
        WHEN increment_by > 0 THEN GREATEST(floor((max_value - last_value) / NULLIF(abs(increment_by), 0)), 0)
        WHEN increment_by < 0 THEN GREATEST(floor((last_value - min_value) / NULLIF(abs(increment_by), 0)), 0)
        ELSE NULL::numeric
      END AS remaining_values,
      CASE
        WHEN max_value > min_value AND increment_by > 0 THEN
          ROUND(GREATEST(max_value - last_value, 0) / NULLIF(max_value - min_value, 0) * 100, 4)
        WHEN max_value > min_value AND increment_by < 0 THEN
          ROUND(GREATEST(last_value - min_value, 0) / NULLIF(max_value - min_value, 0) * 100, 4)
        ELSE NULL::numeric
      END AS headroom_pct,
      CASE
        WHEN cycle THEN 'VERIFY'
        WHEN increment_by = 0 THEN 'VERIFY'
        WHEN max_value IS NULL OR min_value IS NULL THEN 'VERIFY'
        WHEN (
          CASE
            WHEN increment_by > 0 THEN GREATEST(floor((max_value - last_value) / NULLIF(abs(increment_by), 0)), 0)
            WHEN increment_by < 0 THEN GREATEST(floor((last_value - min_value) / NULLIF(abs(increment_by), 0)), 0)
          ...
          OR COALESCE(
               CASE
                 WHEN max_value > min_value AND increment_by > 0 THEN ROUND(GREATEST(max_value - last_value, 0) / NULLIF(max_value - min_value, 0) * 100, 4)
                 WHEN max_value > min_value AND increment_by < 0 THEN ROUND(GREATEST(last_value - min_value, 0) / NULLIF(max_value - min_value, 0) * 100, 4)
                 ELSE NULL::numeric
               END,
               100
             ) <= 1
  ...
  ORDER BY
    CASE status
      ...
      ELSE 4
    END,
    remaining_values NULLS LAST,
    headroom_pct NULLS LAST,
    last_value DESC
  LIMIT 30
) seq_runway;
...
\qecho '<div class="finding info"><div class="finding-body">These rows show child tables whose foreign-key columns do not have a matching left-prefix index. PG360 ranks them by child-table size and scan pressure so ORM-heavy join paths surface first. Suggested DDL is a candidate only; review existing composite indexes before creating anything new.</div></div>'
\qecho '<div class="table-wrap">'
\qecho '<table class="pg360"><thead><tr>'
\qecho '<th>Table</th><th>Constraint</th><th>FK Column(s)</th><th>References</th><th>Table Size</th><th>Seq Scans</th><th>Index Scans</th><th>Suggested Index (DBA review required)</th>'
\qecho '</tr></thead><tbody>'
...
SELECT
  COALESCE(
    string_agg(
      ...
      '<td>' || replace(replace(replace(replace(replace(conname,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '<td class="warn">' || replace(replace(replace(replace(replace(fk_cols_display,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '<td>' || replace(replace(replace(replace(replace(ref_table,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '<td class="num">' || pg_size_pretty(table_bytes) || '</td>' ||
      '<td class="num ' || CASE WHEN seq_scan > 10000 THEN 'crit' WHEN seq_scan > 1000 THEN 'warn' ELSE '' END || '">' || to_char(seq_scan,'FM999,999,999') || '</td>' ||
      '<td class="num">' || to_char(idx_scan,'FM999,999,999') || '</td>' ||
      '<td class="code-block">' || replace(replace(replace(replace(replace(fix_sql,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '</tr>',
      ''
    ),
    '<tr><td colspan="8" class="table-empty">All foreign keys have supporting left-prefix indexes</td></tr>'
  )
FROM (
  WITH fk_base AS (
    SELECT
      ...
      format('%I.%I', n.nspname, c.relname) AS src_table,
      format('%I.%I', rn.nspname, rc.relname) AS ref_table,
      string_agg(a.attname, ', ' ORDER BY cols.ord) AS fk_cols_display,
      string_agg(format('%I', a.attname), ', ' ORDER BY cols.ord) AS fk_cols_sql,
      pg_total_relation_size(c.oid) AS table_bytes,
      COALESCE(st.seq_scan, 0)::bigint AS seq_scan,
      COALESCE(ix.idx_scan, 0)::bigint AS idx_scan
    FROM pg_constraint con
    ...
    LEFT JOIN LATERAL (
      SELECT SUM(sui.idx_scan)::bigint AS idx_scan
      FROM pg_stat_user_indexes sui
      WHERE sui.relid = c.oid
    ) ix ON true
    WHERE con.contype = 'f'
      AND n.nspname NOT IN ('pg_catalog','information_schema')
    GROUP BY con.oid, con.conrelid, con.conname, con.conkey, n.nspname, c.relname, rn.nspname, rc.relname, c.oid, st.seq_scan, ix.idx_scan
  )
  SELECT
    src_table,
    conname,
    ...
    format(
      ...
      'idx_fk_' || substr(md5(src_schema || '.' || src_relname || ':' || fk_cols_sql), 1, 12),
      src_schema,
      src_relname,
      fk_cols_sql
    ) AS fix_sql
  ...
  WHERE NOT EXISTS (
    ...
    WHERE i.indrelid = fk.conrelid
      ...
      AND COALESCE(array_length(i.indkey::smallint[], 1), 0) >= fk.key_len
      AND (i.indkey::smallint[])[1:fk.key_len] = fk.conkey
  )
  ORDER BY table_bytes DESC, seq_scan DESC, src_table
  LIMIT 30
) fk_missing;
...
\qecho '<div class="finding high"><div class="finding-header">'
\qecho '<span class="finding-title">PUBLIC grants should be rare on application-owned tables and views</span>'
...
WITH rel_ext AS (
  SELECT
    c.oid AS relid,
    e.extname,
    COALESCE(av.trusted, false) AS ext_trusted
  FROM pg_class c
  JOIN pg_depend d
    ON d.objid = c.oid
...
SELECT
  COALESCE(
    string_agg(
      ...
      '<td class="' ||
      CASE WHEN c.relkind IN ('r','f') THEN 'crit">Any user can access this relation' ELSE 'warn">Any user can access this view' END ||
      '</td></tr>',
      '' ORDER BY c.relkind, n.nspname, c.relname
    ),
    '<tr><td colspan="4" class="table-empty">No application-owned tables or views found with PUBLIC grants</td></tr>'
  )
FROM pg_class c
JOIN pg_namespace n ON n.oid = c.relnamespace
LEFT JOIN rel_ext rx ON rx.relid = c.oid
WHERE c.relkind IN ('r','v','m','f')
  AND n.nspname NOT IN ('pg_catalog','information_schema')
  AND rx.relid IS NULL
  AND EXISTS (
...
table_grants AS (
  SELECT
    ...
    tp.is_grantable,
    rx.extname,
    COALESCE(rx.ext_trusted, false) AS ext_trusted,
    CASE
      WHEN tp.grantee = 'PUBLIC' AND rx.extname IS NULL THEN 'HIGH'
      WHEN tp.grantee = 'PUBLIC' AND rx.extname IN ('pg_stat_statements') THEN 'MEDIUM'
      WHEN tp.grantee = 'PUBLIC' AND rx.extname IS NOT NULL AND NOT COALESCE(rx.ext_trusted,false) THEN 'MEDIUM'
      WHEN tp.grantee = 'PUBLIC' AND COALESCE(rx.ext_trusted,false) THEN 'LOW'
...
routine_owner AS (
  SELECT
    ...
    array_agg(DISTINCT pe.extname) FILTER (WHERE pe.extname IS NOT NULL) AS ext_names,
    bool_or(COALESCE(pe.ext_trusted, false)) AS ext_trusted
  FROM pg_proc p
  JOIN pg_namespace n ON n.oid = p.pronamespace
  LEFT JOIN proc_ext pe
    ON pe.pronamespace = p.pronamespace
   AND pe.proname = p.proname
  WHERE n.nspname !~ '^pg_'
...
routine_grants AS (
  SELECT
    ...
    rp.privilege_type,
    rp.is_grantable,
    COALESCE((ro.ext_names)[1], NULL) AS extname,
    COALESCE(ro.ext_trusted, false) AS ext_trusted,
    CASE
      WHEN rp.grantee = 'PUBLIC' AND COALESCE((ro.ext_names)[1], NULL) IS NULL THEN 'HIGH'
      WHEN rp.grantee = 'PUBLIC' AND COALESCE((ro.ext_names)[1], '') IN ('dblink','file_fdw','adminpack','hypopg','pg_cron','postgres_fdw') THEN 'HIGH'
      WHEN rp.grantee = 'PUBLIC' AND COALESCE((ro.ext_names)[1], '') IN ('pg_stat_statements','pg_visibility','pgstattuple','pg_buffercache','pg_prewarm') THEN 'MEDIUM'
      WHEN rp.grantee = 'PUBLIC' AND COALESCE(ro.ext_trusted, false) THEN 'LOW'
...
\qecho '    <div class="section-desc">Installed extensions, trusted/common provider baselines, and extension-specific observability guidance.</div>'
\qecho '  </div>'
\qecho '</div>'
...
ext_catalog AS (
  SELECT * FROM (VALUES
    ('pg_stat_statements', 'Diagnostic', 'Required', false),
    ('pg_buffercache',    'Diagnostic', 'Recommended', false),
    ('pgstattuple',       'Diagnostic', 'Recommended', false),
    ('pg_visibility',     'Diagnostic', 'Optional', false),
    ('pg_prewarm',        'Diagnostic', 'Optional', false),
    ('btree_gin',         'Feature',    'Common trusted', true),
    ('citext',            'Feature',    'Common trusted', true),
    ('hstore',            'Feature',    'Common trusted', true),
    ('pg_trgm',           'Feature',    'Common trusted', true),
    ('pgcrypto',          'Security',   'Common trusted', true),
    ('tablefunc',         'Feature',    'Common trusted', true),
    ('unaccent',          'Feature',    'Common trusted', true),
    ('uuid-ossp',         'Feature',    'Common trusted', true)
  ) AS v(ext_name, ext_class, recommendation_tier, pg_trusted_expected)
),
ext_status AS (
  SELECT
    c.ext_name,
    ...
    c.pg_trusted_expected,
    EXISTS (SELECT 1 FROM pg_extension e WHERE e.extname = c.ext_name) AS installed
  FROM ext_catalog c
),
agg AS (
  SELECT
    COUNT(*) FILTER (WHERE recommendation_tier = 'Required') AS required_total,
    COUNT(*) FILTER (WHERE recommendation_tier = 'Required' AND installed) AS required_installed,
    COUNT(*) FILTER (WHERE recommendation_tier = 'Recommended') AS recommended_total,
    COUNT(*) FILTER (WHERE recommendation_tier = 'Recommended' AND installed) AS recommended_installed,
    COUNT(*) FILTER (WHERE recommendation_tier = 'Common trusted') AS trusted_total,
    COUNT(*) FILTER (WHERE recommendation_tier = 'Common trusted' AND installed) AS trusted_installed
  FROM ext_status
),
review AS (
  SELECT
    COUNT(*) FILTER (
      WHERE e.extname NOT IN ('plpgsql')
        AND (
          (COALESCE(av.trusted, false) = false AND EXISTS (
            SELECT 1
            FROM pg_depend d
            JOIN pg_proc p ON p.oid = d.objid
            JOIN pg_language l ON l.oid = p.prolang
          ...
          OR e.extname IN ('hypopg','pg_stat_kcache','pg_hint_plan','pg_visibility','pgstattuple','pg_buffercache','pg_prewarm')
          OR (e.extname = 'pg_stat_statements' AND ((SELECT preload_libs FROM provider_det) NOT ILIKE '%pg_stat_statements%' OR COALESCE((SELECT pgss_track FROM provider_det),'') IN ('', 'none')))
        )
    ) AS admin_review_cnt
  FROM pg_extension e
  LEFT JOIN LATERAL (
...
SELECT
  '<div class="card-grid">' ||
  '<div class="card"><div class="card-label">Detected Platform</div><div class="card-value">' || replace(replace(replace(replace(replace((SELECT provider_guess FROM provider_det),'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</div></div>' ||
  '<div class="card ' || CASE WHEN required_installed = required_total THEN 'good' ELSE 'critical' END || '"><div class="card-label">Required Diagnostic</div><div class="card-value">' || required_installed || '/' || required_total || '</div><div class="card-sub">pg_stat_statements baseline</div></div>' ||
  '<div class="card ' || CASE WHEN recommended_installed = recommended_total THEN 'good' ELSE 'warning' END || '"><div class="card-label">Recommended Diagnostic</div><div class="card-value">' || recommended_installed || '/' || recommended_total || '</div><div class="card-sub">pg_buffercache + pgstattuple</div></div>' ||
  '<div class="card"><div class="card-label">Trusted / Common</div><div class="card-value">' || trusted_installed || '/' || trusted_total || '</div><div class="card-sub">Provider-common baseline</div></div>' ||
  '<div class="card ' || CASE WHEN (SELECT admin_review_cnt FROM review) > 0 THEN 'warning' ELSE 'good' END || '"><div class="card-label">Admin-Review</div><div class="card-value">' || (SELECT admin_review_cnt FROM review) || '</div><div class="card-sub">Untrusted or inspection utilities</div></div>' ||
  '<div class="card ' || CASE WHEN (SELECT track_io_timing FROM provider_det) = 'on' THEN 'good' ELSE 'warning' END || '"><div class="card-label">track_io_timing</div><div class="card-value">' || replace(replace(replace(replace(replace((SELECT track_io_timing FROM provider_det),'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</div></div>' ||
  '</div>'
FROM agg;
...
ext_status AS (
  SELECT * FROM (VALUES
    ...
    ('pg_buffercache', 'Recommended'),
    ('pgstattuple', 'Recommended')
  ) v(ext_name, tier)
),
summary AS (
  SELECT
    COUNT(*) FILTER (WHERE tier = 'Required' AND NOT EXISTS (SELECT 1 FROM pg_extension e WHERE e.extname = ext_name)) AS req_missing,
    COUNT(*) FILTER (WHERE tier = 'Recommended' AND NOT EXISTS (SELECT 1 FROM pg_extension e WHERE e.extname = ext_name)) AS rec_missing,
    (SELECT track_io_timing FROM provider_det) AS track_io_timing,
    (SELECT log_min_duration_statement FROM provider_det) AS log_min_duration_statement,
    (SELECT provider_guess FROM provider_det) AS provider_guess
  FROM ext_status
)
SELECT
  ...
  CASE
    ...
    ELSE 'good'
  END || '">' ||
  '<div class="finding-header"><span class="finding-title">Trusted/common extensions are separated from admin-review utilities</span>' ||
  '<span class="severity-pill ' ||
  CASE
  ...
  END || '</span></div>' ||
  '<div class="finding-body">Provider guess: <strong>' || provider_guess ||
  '</strong>. Official trusted/common extensions are not treated as high-risk just because installation lifecycle is privileged. Admin-review is reserved for untrusted, third-party, inspection, or preload-sensitive extensions.</div></div>'
FROM summary;
...
\qecho '<div class="table-wrap">'
\qecho '<table class="pg360"><thead><tr>'
\qecho '<th>Extension</th><th>Version</th><th>Schema</th><th>Owner</th><th>PG Trust</th><th>Provider Posture</th><th>Review</th><th>Use in PG360 / Operational Note</th>'
\qecho '</tr></thead><tbody>'
...
ext_catalog AS (
  SELECT * FROM (VALUES
    ('pg_stat_statements', 'Diagnostic', 'Required', false, true, true, true, 'Primary SQL telemetry source'),
    ('pg_buffercache',    'Diagnostic', 'Recommended', false, true, true, true, 'Buffer residency validation'),
    ('pgstattuple',       'Diagnostic', 'Recommended', false, true, true, true, 'Exact bloat validation'),
    ('pg_visibility',     'Diagnostic', 'Optional', false, true, true, true, 'Visibility map and vacuum evidence'),
    ('pg_prewarm',        'Diagnostic', 'Optional', false, true, true, true, 'Cache warmup validation'),
    ('pg_stat_kcache',    'Diagnostic', 'Optional', false, false, false, false, 'Third-party OS-level per-query attribution'),
    ('btree_gin',         'Feature',    'Common trusted', true,  true, true, true, 'Trusted GIN operator-class support'),
    ('citext',            'Feature',    'Common trusted', true,  true, true, true, 'Case-insensitive text semantics'),
    ('hstore',            'Feature',    'Common trusted', true,  true, true, true, 'Key-value data type'),
    ('pg_trgm',           'Feature',    'Common trusted', true,  true, true, true, 'Similarity search and LIKE acceleration'),
...
ext AS (
  SELECT
    ...
    e.extversion,
    n.nspname AS ext_schema,
    pg_get_userbyid(e.extowner) AS ext_owner,
    COALESCE(av.superuser, false) AS requires_superuser,
    COALESCE(av.trusted, false) AS is_trusted,
    EXISTS (
      SELECT 1
...
ext_eval AS (
  SELECT
    ...
    CASE
      WHEN e.extname = 'plpgsql' THEN 'LOW'
      WHEN COALESCE(c.recommendation_tier, '') = 'Common trusted' OR e.is_trusted THEN 'LOW'
      WHEN e.extname = 'pg_stat_statements' AND (d.shared_preload_libraries NOT ILIKE '%pg_stat_statements%' OR COALESCE(d.pgss_track, '') IN ('', 'none')) THEN 'MEDIUM'
      WHEN e.extname = 'pg_stat_statements' THEN 'LOW'
      WHEN e.extname IN ('pg_buffercache','pg_prewarm','pg_visibility','pgstattuple','auto_explain','amcheck','pageinspect') THEN 'MEDIUM'
      WHEN e.extname IN ('hypopg','pg_stat_kcache','pg_hint_plan') THEN 'MEDIUM'
      WHEN e.has_c_language AND NOT e.is_trusted THEN 'HIGH'
    ...
    COALESCE(c.purpose,
      CASE e.extname
        ...
        ELSE 'Installed extension outside PG360 baseline catalog'
      END
    ) AS purpose
  FROM ext e
  LEFT JOIN ext_catalog c ON c.extname = e.extname
...
SELECT
  COALESCE(
    string_agg(
      ...
      '<td>' || replace(replace(replace(replace(replace(extversion,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '<td>' || replace(replace(replace(replace(replace(ext_schema,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '<td>' || replace(replace(replace(replace(replace(ext_owner,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '<td>' || pg_trust || '</td>' ||
      ...
      '<td>' || replace(replace(replace(replace(replace(purpose,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '</tr>',
      E'\n' ORDER BY extname
    ),
    '<tr><td colspan="8" class="table-empty">No installed extensions returned in this database.</td></tr>'
  )
FROM ext_eval;
...
\qecho '<div class="subsection">'
\qecho '<div class="subsection-title">Available / Trusted Provider Extension Catalog</div>'
\qecho '<div class="table-wrap">'
\qecho '<table class="pg360"><thead><tr>'
\qecho '<th>Extension</th><th>Available Here</th><th>Installed Here</th><th>Current Version</th><th>PG Trusted</th><th>AWS</th><th>Cloud SQL</th><th>Azure</th><th>PG360 Tier</th><th>Why It Matters</th>'
\qecho '</tr></thead><tbody>'
...
WITH provider_catalog AS (
  SELECT * FROM (VALUES
    ...
    (9, 'hstore', 'Common trusted', 'Trusted/common', 'Supported', 'Trusted/common', 'Key-value data type'),
    (10, 'pg_trgm', 'Common trusted', 'Trusted/common', 'Supported', 'Trusted/common', 'Similarity search and LIKE acceleration'),
    (11, 'pgcrypto', 'Common trusted', 'Trusted/common', 'Supported', 'Trusted/common', 'Cryptographic functions'),
    (12, 'tablefunc', 'Common trusted', 'Trusted/common', 'Supported', 'Trusted/common', 'Crosstab and helper functions'),
    (13, 'unaccent', 'Common trusted', 'Trusted/common', 'Supported', 'Trusted/common', 'Accent-insensitive search'),
    (14, 'uuid-ossp', 'Common trusted', 'Trusted/common', 'Supported', 'Trusted/common', 'UUID generation')
  ) AS v(sort_key, ext_name, pg360_tier, aws_note, cloudsql_note, azure_note, purpose)
),
avail AS (
  SELECT
    ae.name AS ext_name,
    ae.default_version,
    EXISTS (SELECT 1 FROM pg_extension e WHERE e.extname = ae.name) AS installed,
    COALESCE((
      SELECT bool_or(aev.trusted)
      FROM pg_available_extension_versions aev
      WHERE aev.name = ae.name
    ), false) AS pg_trusted
...
SELECT
  COALESCE(
    string_agg(
      '<tr>' ||
      '<td>' || replace(replace(replace(replace(replace(pc.ext_name,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;') || '</td>' ||
      '<td class="' || CASE WHEN a.ext_name IS NOT NULL THEN 'good">Yes' ELSE 'warn">No' END || '</td>' ||
      '<td class="' || CASE WHEN COALESCE(a.installed,false) THEN 'good">Yes' ELSE 'warn">No' END || '</td>' ||
      '<td>' || COALESCE(replace(replace(replace(replace(replace(a.default_version,'&','&amp;'),'<','&lt;'),'>','&gt;'),'"','&quot;'),'''','&#39;'),'n/a') || '</td>' ||
      '<td>' || CASE WHEN COALESCE(a.pg_trusted,false) THEN 'Yes' ELSE 'No' END || '</td>' ||
      '<td>' || pc.aws_note || '</td>' ||
      '<td>' || pc.cloudsql_note || '</td>' ||
      '<td>' || pc.azure_note || '</td>' ||
      '<td>' || pc.pg360_tier || '</td>' ||
      '<td>' || pc.purpose || '</td>' ||
      '</tr>',
      E'\n' ORDER BY pc.sort_key, pc.ext_name
    ),
    '<tr><td colspan="10" class="table-empty">No provider extension baseline catalog available.</td></tr>'
  )
FROM provider_catalog pc
LEFT JOIN avail a ON a.ext_name = pc.ext_name;
...
\qecho '  }'
\qecho ''
\qecho '  function copyTextToClipboard(text) {'
\qecho '    if (navigator.clipboard && navigator.clipboard.writeText) {'
\qecho '      return navigator.clipboard.writeText(text);'
\qecho '    }'
\qecho '    return new Promise(function (resolve, reject) {'
\qecho '      var ta = document.createElement(''textarea'');'
\qecho '      ta.value = text;'
\qecho '      ta.setAttribute(''readonly'', ''readonly'');'
\qecho '      ta.style.position = ''absolute'';'
\qecho '      ta.style.left = ''-9999px'';'
\qecho '      document.body.appendChild(ta);'
\qecho '      ta.select();'
\qecho '      try {'
\qecho '        document.execCommand(''copy'');'
\qecho '        document.body.removeChild(ta);'
\qecho '        resolve();'
\qecho '      } catch (err) {'
\qecho '        document.body.removeChild(ta);'
\qecho '        reject(err);'
...
\qecho '  }'
\qecho ''
\qecho '  function extractCopyText(el) {'
\qecho '    var clone = el.cloneNode(true);'
\qecho '    Array.prototype.forEach.call(clone.querySelectorAll(''.copy-button''), function (btn) {'
\qecho '      btn.remove();'
\qecho '    });'
\qecho '    return (clone.innerText || clone.textContent || '''').trim();'
\qecho '  }'
\qecho ''
\qecho '  function flashCopyButton(button, host) {'
\qecho '    var original = button.getAttribute(''data-label'') || ''Copy'';'
\qecho '    button.textContent = ''Copied'';'
\qecho '    button.classList.add(''copied'');'
\qecho '    host.classList.add(''copy-flash'');'
\qecho '    window.setTimeout(function () {'
\qecho '      button.textContent = original;'
\qecho '      button.classList.remove(''copied'');'
\qecho '      host.classList.remove(''copy-flash'');'
\qecho '    }, 900);'
\qecho '  }'
\qecho ''
...
\qecho '      if (el.querySelector(''.copy-button'')) {'
\qecho '        return;'
\qecho '      }'
\qecho '      el.classList.add(''copyable'');'
\qecho '      var button = document.createElement(''button'');'
\qecho '      button.type = ''button'';'
\qecho '      button.className = ''copy-button'';'
\qecho '      button.textContent = ''Copy'';'
\qecho '      button.setAttribute(''data-label'', ''Copy'');'
\qecho '      button.setAttribute(''aria-label'', ''Copy SQL or code block'');'
\qecho '      button.addEventListener(''click'', function (evt) {'
\qecho '        evt.preventDefault();'
\qecho '        evt.stopPropagation();'
\qecho '        copyTextToClipboard(extractCopyText(el)).then(function () {'
\qecho '          flashCopyButton(button, el);'
\qecho '        }).catch(function () {'
\qecho '          /* no-op */'
\qecho '        });'
\qecho '      });'
\qecho '      el.insertBefore(button, el.firstChild);'
\qecho '    });'
\qecho '  }'
\qecho ''
\qecho ''
...
\qecho '  }'
\qecho ''
\qecho '  var tableSequence = 0;'
\qecho ''
\qecho '  function ensureTableId(table) {'
\qecho '    if (!table.id) {'
\qecho '      tableSequence += 1;'
\qecho '      table.id = ''pg360_table_'' + String(tableSequence);'
\qecho '    }'
\qecho '    return table.id;'
\qecho '  }'
\qecho ''
\qecho '  function getDataRows(table) {'
\qecho '    return Array.prototype.filter.call(table.querySelectorAll(''tbody tr''), function (row) {'
\qecho '      return !row.querySelector(''.table-empty'');'
\qecho '    });'
\qecho '  }'
\qecho ''
\qecho '  function getFilterInputForTable(table) {'
\qecho '    if (!table || !table.id) {'
\qecho '      return null;'
\qecho '    }'
\qecho '    return document.querySelector(''.table-search[data-table-target="'' + table.id + ''"]'');'
\qecho '  }'
\qecho ''
\qecho '  function ensureTableRowToggle(table, totalRows) {'
\qecho '    var wrap = table.closest(''.table-wrap'');'
\qecho '    if (!wrap || totalRows <= 15) {'
\qecho '      return null;'
\qecho '    }'
\qecho '    var toolbar = wrap.querySelector(''.table-toolbar'');'
\qecho '    if (!toolbar) {'
\qecho '      toolbar = document.createElement(''div'');'
\qecho '      toolbar.className = ''table-toolbar'';'
\qecho '      wrap.insertBefore(toolbar, wrap.firstChild);'
\qecho '    }'
\qecho '    var button = toolbar.querySelector(''.table-row-toggle'');'
\qecho '    if (!button) {'
\qecho '      button = document.createElement(''button'');'
\qecho '      button.type = ''button'';'
\qecho '      button.className = ''table-row-toggle'';'
\qecho '      button.addEventListener(''click'', function () {'
\qecho '        table.dataset.rowsExpanded = table.dataset.rowsExpanded === ''true'' ? ''false'' : ''true'';'
\qecho '        syncTableRowVisibility(table);'
\qecho '      });'
\qecho '      toolbar.appendChild(button);'
\qecho '    }'
\qecho '    return button;'
\qecho '  }'
\qecho ''
\qecho '  function syncTableRowVisibility(table) {'
\qecho '    var limit = parseInt(table.getAttribute(''data-row-collapse-limit'') || ''15'', 10);'
\qecho '    var rows = getDataRows(table);'
\qecho '    if (!rows.length) {'
\qecho '      return;'
\qecho '    }'
\qecho '    var button = ensureTableRowToggle(table, rows.length);'
\qecho '    var filterInput = getFilterInputForTable(table);'
\qecho '    var filterActive = !!(filterInput && filterInput.value && filterInput.value.trim());'
\qecho '    var expanded = table.dataset.rowsExpanded === ''true'';'
\qecho '    var visibleIndex = 0;'
\qecho '    var matchingRows = 0;'
\qecho ''
\qecho '    rows.forEach(function (row) {'
\qecho '      var matches = (row.dataset.pg360FilterMatch || ''1'') !== ''0'';'
\qecho '      if (!matches) {'
\qecho '        row.hidden = true;'
\qecho '        row.classList.remove(''row-collapsed'');'
\qecho '        return;'
\qecho '      }'
\qecho '      matchingRows += 1;'
\qecho '      if (!expanded && !filterActive && visibleIndex >= limit) {'
\qecho '        row.hidden = true;'
\qecho '        row.classList.add(''row-collapsed'');'
\qecho '      } else {'
\qecho '        row.hidden = false;'
\qecho '        row.classList.remove(''row-collapsed'');'
\qecho '      }'
\qecho '      visibleIndex += 1;'
\qecho '    });'
\qecho ''
\qecho '    if (button) {'
\qecho '      if (filterActive) {'
\qecho '        button.hidden = false;'
\qecho '        button.disabled = true;'
\qecho '        button.textContent = matchingRows + '' matching rows'';'
\qecho '      } else {'
\qecho '        button.hidden = false;'
\qecho '        button.disabled = false;'
\qecho '        button.textContent = expanded ? ''Show first '' + limit + '' rows'' : ''Show all '' + rows.length + '' rows'';'
\qecho '      }'
\qecho '    }'
\qecho '  }'
\qecho ''
\qecho '  function initLargeTables() {'
\qecho '    Array.prototype.forEach.call(document.querySelectorAll(''table.pg360''), function (table) {'
\qecho '      ensureTableId(table);'
\qecho '      var rows = getDataRows(table);'
\qecho '      rows.forEach(function (row) {'
\qecho '        row.dataset.pg360FilterMatch = ''1'';'
\qecho '      });'
\qecho '      if (rows.length > 15) {'
\qecho '        table.setAttribute(''data-row-collapse-limit'', ''15'');'
\qecho '        if (!table.hasAttribute(''data-rows-expanded'')) {'
\qecho '          table.dataset.rowsExpanded = ''false'';'
\qecho '        }'
\qecho '        syncTableRowVisibility(table);'
\qecho '      }'
\qecho '    });'
...
\qecho '      if (siblingTable) {'
\qecho '        targetId = ensureTableId(siblingTable);'
\qecho '      }'
\qecho '    }'
\qecho '    if (!targetId) {'
\qecho '      return;'
...
\qecho '    getDataRows(table).forEach(function (row) {'
\qecho '      var matches = !filter || row.textContent.toLowerCase().indexOf(filter) !== -1;'
\qecho '      row.dataset.pg360FilterMatch = matches ? ''1'' : ''0'';'
\qecho '    });'
\qecho '    syncTableRowVisibility(table);'
\qecho '  }'
\qecho ''
